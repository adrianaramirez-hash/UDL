...
def _load_from_gsheets(sheet_id: str):
    ...
    ws_resp = sh.worksheet("Respuestas")
    ws_map = sh.worksheet("Mapa_Preguntas")
    ws_cat = sh.worksheet("Catalogo_Servicio")
...
def render_encuesta_calidad(vista: str, carrera: str | None):
    ...
    if not required_cols.issubset(set(mapa.columns)):
        st.error("La hoja 'Mapa_Preguntas' no trae las columnas esperadas: header_exacto, scale_code, header_num.")
        return
    ...
    with st.sidebar:
        ...
        # Programa (solo si vista Dirección General; en Director de carrera lo evitamos para no duplicar decisiones)
        ...
        # Carrera (solo para Dirección General, como drill-down opcional).
        # Para Director de carrera, se aplica automático (bloqueado).
    ...
    # Carrera automática en vista Director de carrera
    ...
    with tab1:
        ...
        if not qdf.empty:
            # Orden: por sección y luego por promedio
    ...
    with tab3:
        ...
        # Heurística: columnas con comentarios / ¿Por qué? / sugerencias
        open_cols = [
