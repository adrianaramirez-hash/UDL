...
def _create_excel_download(df: pd.DataFrame, kpis_df: pd.DataFrame | None, filename: str):
    output = BytesIO()
...
def _download_buttons_body(df: pd.DataFrame, filename_prefix: str, kpis_df: pd.DataFrame | None = None):
...
def render_encuesta_calidad(vista: str | None = None, carrera: str | None = None):
    ...
    # =========================================================
    # VISTA FINANZAS (mismo dashboard, mapa sintético por prefijo)
    # =========================================================
    if vista == "Dirección Finanzas":
        st.caption("Vista restringida para Dirección de Finanzas (solo datos administrativos autorizados).")
        ...
        # filtro opcional adicional (si existe alguna columna tipo Servicio/Programa/Modalidad)
        ...
        # detectar numéricas (sin depender de *_num)
        base_exclude = set([c for c in ["Marca temporal", "Marca Temporal"] if c in f.columns])
        num_candidates = []
        for c in f.columns:
        ...
        # abiertas sintéticas por prefijo
        open_items_all = []
        for c in open_cols:
            sec = _section_from_numcol(str(c))
        ...
        with tab1:
            ...
            if rows:
                ...
                sec_chart = _bar_chart_auto(
                    ...
                    value_title="Promedio",
                    tooltip_cols=[
                        alt.Tooltip("__cat:N", title="Sección"),
                        alt.Tooltip("__val:Q", title="Promedio", format=".2f"),
                        ...
                        if qdf.empty:
                            ...
                            if not qdf_l.empty:
                                ...
                                chart_l = _bar_chart_auto(
                                    ...
                                    value_title="Promedio",
                                    tooltip_cols=[
                                        alt.Tooltip("__cat:N", title="Pregunta"),
                                        alt.Tooltip("__val:Q", title="Promedio", format=".2f"),
                                    ],
                                    max_vertical=MAX_VERTICAL_QUESTIONS,
                                    wrap_width_vertical=24,
                            ...
                            if not qdf_y.empty:
                                ...
                                chart_y = _bar_chart_auto(
                                    ...
                                    tooltip_cols=[
                                        alt.Tooltip("__cat:N", title="Pregunta"),
                                        alt.Tooltip("__val:Q", title="% Sí", format=".1f"),
                                    ],
                                    max_vertical=MAX_VERTICAL_QUESTIONS,
                                    wrap_width_vertical=24,
                                    wrap_width_horizontal=40,
    ...
    # =========================================================
    # VISTA GENERAL / DIRECTORES (tu lógica original)
    # =========================================================
    modalidad = _resolver_modalidad_auto(vista, carrera) if vista != "Dirección General" else None
                ...
                for (sec_code, sec_name), g in mapa_ok_num.groupby(["section_code", "section_name"]):
                    ...
                    with st.expander(str(sec_name), expanded=False):
                        ...
                        chart = _bar_chart_auto(
                            ...
                            value_title="Promedio",
                            tooltip_cols=[
                                alt.Tooltip("__cat:N", title="Carrera/Servicio"),
                                alt.Tooltip("__val:Q", title="Promedio", format=".2f"),
