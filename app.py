...
# ✅ NUEVO: import defensivo del módulo Capacitación Docente
try:
...
# ✅ Import defensivo del módulo Encuesta de Calidad DF
try:
...
# ✅ Import defensivo del módulo de Bajas/Retención
try:
...
# ✅ Import defensivo del módulo Seguimiento de Inscripciones
try:
...
# ============================================================
# Normalización de UNIDADES compactadas
# ============================================================
def _slug(s: str) -> str:
...
def resolver_permiso_por_email(email: str, df_accesos: pd.DataFrame) -> dict:
    ...
    if rol == "DC" and not servicios:
        return {
            ...
            "servicios": [],
            "modulos": set(),
            "mensaje": "Falta SERVICIO_ASIGNADO para ROL=DC.",
        }
    ...
    if not modulos:
        return {
            ...
            "servicios": [],
            "modulos": set(),
            "mensaje": "Tu usuario no tiene MODULOS asignados en ACCESOS.",
        }
...
if "user_rol" not in st.session_state:
    ...
    try:
        ...
        try:
            ...
            sa_email = ""
        if sa_email:
            st.info(f"Comparte el Sheet de ACCESOS con este correo como Viewer: {sa_email}")
...
try:
    if seccion == "Encuesta de calidad":
        if vista == "Dirección Finanzas":
            if (
                not HAS_EC_F_MOD
                or encuesta_calidad_f is None
                or not hasattr(encuesta_calidad_f, "render_encuesta_calidad_f")
            ):
                st.subheader("Encuesta de calidad — Finanzas")
                st.error("El módulo DF no está disponible o no expone `render_encuesta_calidad_f`.")
                st.stop()
        ...
        if not HAS_CAPACITACION_MOD or capacitacion_docente is None:
            st.subheader("Capacitación Docente")
            st.error("El módulo `capacitacion_docente.py` no se pudo importar.")
            st.caption("Revisa que el archivo exista en la raíz del repo y no tenga errores de sintaxis.")
            _show_traceback_expander("Detalle técnico Capacitación Docente")
            st.stop()

        if hasattr(capacitacion_docente, "render_capacitacion_docente"):
            capacitacion_docente.render_capacitacion_docente(vista=vista, carrera=carrera)
            ...
            st.subheader("Capacitación Docente")
            st.error(
                "El archivo `capacitacion_docente.py` no tiene una función compatible."
            )
            st.caption(
                "Debe tener `render_capacitacion_docente(vista, carrera)` "
                "o `mostrar_modulo_capacitacion_docente()`."
            )
            st.stop()
        ...
        try:
            ...
            _show_traceback_expander("Detalle técnico Aulas virtuales")
            st.stop()
        ...
        if not HAS_SEGUIMIENTO_INS_MOD or seguimiento_inscripciones is None:
            ...
            if not hasattr(seguimiento_inscripciones, "render_seguimiento_inscripciones"):
                st.subheader("Seguimiento de Inscripciones")
                st.error(
                    "El módulo `seguimiento_inscripciones.py` no tiene la función "
                    "`render_seguimiento_inscripciones`."
                )
            else:
