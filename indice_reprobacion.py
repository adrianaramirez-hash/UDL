...
def _ciclo_to_int(x) -> int | None:
    ...
    if not s or s in ["(Todos)", "(Todas)", "Todos", "Todas"]:
        return None
...
def preparar_reprobacion(df: pd.DataFrame, umbral_reprobacion: float = 70) -> pd.DataFrame:
    ...
    for c in ["CICLO", "ESCUELA", "NIVEL", "AREA", "MATRICULA", "ALUMNO", "MATERIA"]:
        ...
        df[c] = df[c].replace(["nan", "None", "NaN", "<NA>", ""], defaults.get(c, ""))
    ...
    df = df[df["CALIF_FINAL"].notna()].copy()
    df = df[df["CALIF_FINAL"] < umbral_reprobacion].copy()
...
def grafica_carreras(resumen: pd.DataFrame):
    if resumen.empty:
        return alt.Chart(pd.DataFrame({"Carrera": [], "Alumnos únicos con reprobación": []})).mark_bar()
...
def grafica_top_materias(tabla: pd.DataFrame):
    if tabla.empty:
        return alt.Chart(pd.DataFrame({"Materia": [], "Alumnos únicos con reprobación": []})).mark_bar()
...
def grafica_historico(hist: pd.DataFrame, titulo: str):
    if hist.empty:
        return alt.Chart(pd.DataFrame({"Ciclo": [], "Alumnos únicos con reprobación": []})).mark_line()
...
def render_indice_reprobacion(vista: str | None = None, carrera: str | None = None):
    ...
    if df.empty:
        ...
        st.caption("Revisa que la hoja tenga la columna CALIF FINAL y que tenga valores numéricos menores a 70.")
        return
    ...
    if vista_modulo == "Resumen ejecutivo":
        ...
        if len(resumen) < 2:
            st.info("Solo hay una carrera en el filtro actual. Para comparar, selecciona '(Todas)' en Programa académico.")
        else:
